...
| `AI_COMMITMENT_STALE_FLOOR_ENABLED` | 1.1 "stale floor" slip: once per run, before any finding, he describes the previous floor's anomaly place as if it were this one. Needs the client's `previous_anomaly_detail` (clients ≤ v1.0.5 never send it, so `true` on `main` is inert). Default `true`. |
| `AI_COMMITMENT_STALE_FLOOR_CHANCE` | `0.0`–`1.0`, default `0.35`. Per-floor chance of the stale-floor slip on an eligible floor. Same floor never re-rolls. |
